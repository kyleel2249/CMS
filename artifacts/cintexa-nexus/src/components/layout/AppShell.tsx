    ...
    <div className="flex h-screen w-full bg-background overflow-hidden text-foreground selection:bg-primary selection:text-primary-foreground">
      ...
      >
        <div className="h-16 flex items-center justify-between px-4 border-b border-sidebar-border shrink-0">
          {!collapsed && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2 overflow-hidden">
              ...
              <span className="font-bold text-lg tracking-wider bg-clip-text text-transparent bg-gradient-to-r from-primary to-secondary">NEXUS</span>
            </motion.div>
          )}
          {collapsed && (
              ...
              {group.items.map((navItem) => {
                ...
                return (
                  <Link key={navItem.href} href={navItem.href} className="block">
                    <div className={cn("flex items-center gap-3 px-3 py-2 rounded-md cursor-pointer transition-all duration-200 group relative", isActive ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground")}>
                      {isActive && <motion.div layoutId="activeTab" className="absolute left-0 top-0 bottom-0 w-1 bg-primary rounded-r-md shadow-[0_0_10px_rgba(0,255,255,0.5)]" />}
                      <navItem.icon className={cn("w-4 h-4 shrink-0", isActive ? "text-primary drop-shadow-[0_0_8px_rgba(0,255,255,0.5)]" : "")} />
                      {!collapsed && <span className="font-medium text-sm whitespace-nowrap">{navItem.label}</span>}
                      {collapsed && (
        ...
        <div className="p-3 border-t border-sidebar-border shrink-0">
          <button onClick={() => setCollapsed(!collapsed)} className="w-full flex items-center justify-center p-2 rounded hover:bg-sidebar-accent text-muted-foreground transition-colors">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="h-16 border-b border-border bg-background/50 backdrop-blur-md flex items-center justify-between px-6 z-10 shrink-0">
          <button onClick={() => setCmdOpen(true)} className="flex items-center bg-muted/50 rounded-full px-3 py-1.5 w-80 border border-border hover:border-primary/50 hover:ring-1 hover:ring-primary/20 transition-all text-left">
            <Search className="w-4 h-4 text-muted-foreground mr-2 shrink-0" />
            <span className="text-sm w-full text-muted-foreground/70 font-mono">Query NEXUS...</span>
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setNotifOpen(true)} className="relative text-muted-foreground hover:text-foreground transition-colors p-1.5 rounded-md hover:bg-muted/50">
              <Bell className="w-5 h-5" />
              <span className="absolute top-0.5 right-0.5 w-2 h-2 bg-primary rounded-full shadow-[0_0_8px_rgba(0,255,255,0.8)]" />
            </button>
            <button onClick={() => setSettingsOpen(true)} className="text-muted-foreground hover:text-foreground transition-colors p-1.5 rounded-md hover:bg-muted/50">
              <Settings className="w-5 h-5" />
            </button>
            <button onClick={() => setSettingsOpen(true)} className="w-8 h-8 rounded-full bg-secondary/20 border border-secondary flex items-center justify-center text-sm font-bold text-secondary hover:bg-secondary/30 transition-colors">
              EX
            </button>
          </div>
        </header>
